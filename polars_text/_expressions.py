...
type Segmentation = Literal["automatic", "line", "sentence"]
...
def word_count(expr: pl.Expr) -> pl.Expr:
    return register_plugin_function(
        plugin_path=PLUGIN_PATH,
...
def char_count(expr: pl.Expr) -> pl.Expr:
    return expr.cast(pl.String).str.len_chars().fill_null(0).cast(pl.Int64)


def sentence_count(expr: pl.Expr) -> pl.Expr:
    return register_plugin_function(
        plugin_path=PLUGIN_PATH,
...
def topic_modeling(
    ...
    *,
    ...
    segmentation: Segmentation = "automatic",
    max_tokens: int = 256,
    seed: int = 42,
